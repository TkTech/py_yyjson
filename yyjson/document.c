...
Document_init(DocumentObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (PyObject_IsInstance(content, path)) {
        ...
        if (!self->m_doc) {
            PyErr_SetString(PyExc_ValueError, "Unable to create empty mutable document.");
            return -1;
        }
...
Document_dumps(DocumentObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (self->i_doc) {
        ...
        if (pointer) {
            val_to_serialize = yyjson_doc_ptr_getn(
                self->i_doc,
                pointer,
                pointer_size
            );
        ...
        if (pointer) {
            mut_val_to_serialize = yyjson_mut_doc_ptr_getn(
                self->m_doc,
                pointer,
                pointer_size
            );
...
Document_get_pointer(DocumentObject *self, PyObject *args)
{
    ...
    if (self->i_doc) {
        yyjson_val *result = yyjson_doc_ptr_getx(
            self->i_doc,
            pointer,
            pointer_len,
            &err
        ...
        if (!result) {
            PyErr_SetString(PyExc_ValueError, err.msg ? err.msg : "Not a valid JSON Pointer");
            return NULL;
        }

        return element_to_primitive(result);
    } else {
        yyjson_mut_val *result = yyjson_mut_doc_ptr_getx(
            self->m_doc,
            pointer,
            pointer_len,
            NULL,
...
static PyObject *
Document_freeze(DocumentObject *self)
{
    if (self->m_doc) {
...
Document_merge_patch(DocumentObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (self->i_doc) {
        ...
        if (pointer != NULL) {
            ...
            if (!original) {
                PyErr_SetString(PyExc_ValueError, ptr_err.msg ? ptr_err.msg : "Not a valid JSON Pointer");
                return NULL;
            }
        } else {
        ...
        if (patch_doc->m_doc) {
            patch_doc->i_doc = yyjson_mut_doc_imut_copy(patch_doc->m_doc, patch_doc->alc);
...
static PyMethodDef Document_methods[] = {
    ...
    {"freeze",
        (PyCFunction)(void(*)(void))Document_freeze,
        METH_NOARGS,
        Document_freeze_doc
    },
    {"merge_patch",
...
static PyGetSetDef Document_members[] = {
    ...
    {"is_mutable",
        ...
        NULL,
        "Returns whether the Document is mutable.",
        NULL
    },
    {NULL} /* Sentinel */
};
